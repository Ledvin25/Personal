...
def validar_fecha_evento(fecha_inicial, fecha_final):
    ...
    try:
        datetime.datetime.strptime(fecha_final, '%d/%m/%Y') # Verificar que la fecha final sea un string con formato 'dd/mm/aaaa'
        datetime.datetime.strptime(fecha_inicial, '%d/%m/%Y') # Verificar que la fecha inicial sea un string con formato 'dd/mm/aaaa'

        fecha1_datetime = datetime.strptime(fecha_inicial, '%d/%m/%Y').date()
        fecha2_datetime = datetime.strptime(fecha_final, '%d/%m/%Y').date()

        if fecha1_datetime > fecha_inicial:
            return True
        else:
            return False
    except ValueError:
