...
import validate_email_address # Libreria para validar el correo electronico
...
def send_email(from_addr, to_addr, subject, body, password):
    ...
    # enviar mensaje a través de SMTP
...
def verify_email_exists(email_addr, password):
    # establecer conexión IMAP
    mail = imaplib.IMAP4_SSL('imap.gmail.com')
    mail.login(email_addr, password)
    mail.select('inbox')

    # buscar mensajes de error de entrega
    result, data = mail.uid('search', None, 'BODY "Delivery to the following recipient failed permanently"')

    # si no hay mensajes de error, entonces el correo existe
    if result == 'OK' and data[0]:
        print(f"El correo {email_addr} no existe.")
    else:
        print(f"El correo {email_addr} existe.")
    
    # cerrar conexión
    mail.logout()

# ejemplo de uso
from_addr = 'exitlagledvin25@gmail.com'
to_addr = 'correo.destinatario@gmail.com'
subject = 'Prueba'
body = 'Este es un correo de prueba.'
password = 'mi_contraseña'

send_email(from_addr, to_addr, subject, body, password)
verify_email_exists(to_addr, password)


print(verificar_correo_electronico('led_25.ok@hotmail.es'))
