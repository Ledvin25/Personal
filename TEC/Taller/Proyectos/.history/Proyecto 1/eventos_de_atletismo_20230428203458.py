...
def mejores_marcas_por_prueba():
    ...
    while not validar_fecha_evento(fecha_inicio) or not validar_fecha_evento(fecha_fin):
        print('Fecha invalida')
        fecha_inicio = input('Ingrese la fecha de inicio (dd/mm/aaaa): ')
        fecha_fin = input('Ingrese la fecha de fin (dd/mm/aaaa): ')
    ...
    # encontrar pruebas dentro del rango de fechas y agregarlas a la lista datos_finales: nombre del evento, nombre de la prueba, codigo de la prueba

    for evento in eventos:

        if fecha_inicio <= evento[2] <= fecha_fin:
            for prueba in pruebas:
                if prueba[0] == evento[1]:
                    datos_finales.append([evento[0], prueba[0], prueba[1]])
