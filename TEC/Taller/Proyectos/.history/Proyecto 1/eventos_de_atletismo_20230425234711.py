...
def ordernar_marcas():
    ...
    # asignar lugar a cada marca y el atleta que la realizo
    ...
    # ordenar marcas de mayor a menor si la medicion es 'M'

    for elemento in marcas_por_evento:
        for elemento1 in elemento[1:]:
            if elemento1[1][0][2] == 'M':
                elemento1[1][1:] = sorted(elemento1[1][1:], key=lambda x: x[2], reverse=True)

    # ordenar marcas de menor a mayor si la medicion es 'T'

    for elemento in marcas_por_evento:
        for elemento1 in elemento[1:]:
            if elemento1[1][0][2] == 'T':
                elemento1[1][1:] = sorted(elemento1[1][1:], key=lambda x: x[2])
