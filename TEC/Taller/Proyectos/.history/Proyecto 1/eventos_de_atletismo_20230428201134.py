...
def mejores_marcas_por_prueba():
    ...
    # Crear un lienzo
    pdf = canvas.Canvas(ruta_escritorio + '/mejores_marcas_por_prueba.pdf')
    pdf.drawString(50, 800, "APLICACIÓN EVENTOS DE ATLETISMO")
    pdf.drawString(50, 785, "MEJORES MARCAS POR PRUEBA")
    pdf.drawString(50, 770, "MEJORES MARCAS POR PRUEBA EN UN RANGO DE FECHAS")
    ...
    for elemento in marcas_por_evento:
        for elemento2 in elemento[1:]:
            if elemento2[0] not in pruebas:
                pruebas.append(elemento2[0])
    ...
    for elemento in marcas:
        if elemento[0] >= fecha_inicial and elemento[0] <= fecha_final:
            marcas_finales.append(elemento)
    ...
    for prueba in pruebas:
        for marca in marcas_finales:
            for marca2 in marca[1:]:
                if marca2[0] == prueba:
                    for marca3 in marca2[1:]:
                        mejores_marcas.append([marca[0], marca2[0], marca3[0], marca3[1], marca3[2], marca3[3]])

    # Ordenar las mejores marcas

    mejores_marcas.sort(key=lambda x: x[2])

    # Agregar al pdf las mejores marcas

    yspace = 740

    for elemento in mejores_marcas:

        yspace -= 30

        pdf.drawString(50, yspace, 'Fecha: ' + elemento[0])
        pdf.drawString(190, yspace, 'Prueba: ' + elemento[1])
        pdf.drawString(330, yspace, 'Marca: ' + str(dar_formato_marca(elemento[2], tipo_medicion(elemento[1]))))
        pdf.drawString(470, yspace, 'Posicion: ' + str(elemento[5]))
