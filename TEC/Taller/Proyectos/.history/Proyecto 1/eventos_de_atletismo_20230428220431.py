...
atletas = [ ['3123456789', 'Pedro', 'Pérez', 'Peraza', 'M', 'CRI', '15/10/2000', 'amataled25@gmail.com', '55556789' ], ['3023456789', 'Juan', 'Pérez', 'Peraza', 'M', 'CRI', '15/10/2000', 'led_25.ok@hotmail.com', '55556789' ], ['2023071280', 'Ledvin', 'Leiva', 'Mata', 'M', 'CRI', '25/12/2004', 'ledvin25@hotmail.com', '55556789' ]]
...
# validar rango de fechas, que la fecha1 este dentro de fecha2 y fecha3

def validar_rango_fechas(fecha1, fecha2, fecha3):

    fecha1 = fecha1.split('/')
    fecha2 = fecha2.split('/')
    fecha3 = fecha3.split('/')

    if int(fecha1[2]) >= int(fecha2[2]) and int(fecha1[2]) <= int(fecha3[2]):
        if int(fecha1[1]) >= int(fecha2[1]) and int(fecha1[1]) <= int(fecha3[1]):
            if int(fecha1[0]) >= int(fecha2[0]) and int(fecha1[0]) <= int(fecha3[0]):
                return True
            else:
                return False
        else:
...
def marcas_por_altleta_correo(identificacion):
        
    marcas = ordernar_marcas()
        
    # Crear un lienzo
    pdf = canvas.Canvas(ruta_escritorio + '/marcas_por_atleta.pdf')
    pdf.drawString(50, 800, "APLICACIÓN EVENTOS DE ATLETISMO")
    pdf.drawString(50, 785, "MARCAS POR ATLETA")
    pdf.drawString(50, 770, "MARCAS DE UN ATLETA ESPECIFICO")

    datos_finales = []

    # agregar al pdf identificacion y nombre del atleta

    # verificar que el atlteta exista

    verify = True

    while verify:
        for nombre in atletas:
            if identificacion == nombre[0]:
                nombre = ' '.join(nombre[1:4])
                verify = False
                break
        if verify:
            identificacion = input('Atleta no encontrado, ingrese una identificacion valida: ')

        
    # buscar los eventos en los que participo el atleta y agregarlos a la lista datos_finales: la identificacion del evento, el nombre del evento y el codigo de las pruebas en las que participo

    for elemento in marcas:
        for elemento2 in elemento[1:]:
            for elemento3 in elemento2[1:]:
                if elemento3[1] == nombre:
                    for evento in eventos:
                        if evento[0] == elemento[0]:
                            datos_finales.append([evento[0], elemento2[0], evento[1]])
                            break

    # agregar al pdf agrupados por prueba los eventos en los que participo el atleta

    yspace_atleta = 740

    pdf.drawString(50, yspace_atleta, 'Atleta: ' + nombre)

    for elemento in datos_finales:
        for id_evento in marcas_por_evento:
            if id_evento[0] == elemento[0]:
                # para cada prueba en la que participo el atleta
                for prueba in pruebas:
                    if prueba[0] == elemento[1]:

                        yspace_prueba = yspace_atleta - 15

                        pdf.drawString(50, yspace_prueba, 'Prueba: ' + prueba[0])
                        pdf.drawString(190, yspace_prueba, prueba[1])
                        pdf.drawString(330, yspace_prueba, 'Categoria: ' + prueba[2])
                        pdf.drawString(470, yspace_prueba, 'Sexo: ' + prueba[3])
                            
                        # Agregar texto: Nombre del evento, marca y poscion en el evento

                        pdf.drawString(50, yspace_prueba - 30, 'Nombre del evento')
                        pdf.drawString(285, yspace_prueba - 30, 'Marca')
                        pdf.drawString(470, yspace_prueba - 30, 'Posicion')

                        # agregar al pdf la marca: nombre del evento, marca y posicion en el evento

                        yspace_prueba -= 30

                        for marca in marcas:
                            if marca[0] == elemento[0]:
                                for marca2 in marca[1:]:
                                    if marca2[0] == elemento[1]:
                                        for marca3 in marca2[1:]:
                                            if marca3[1] == nombre:
                                                yspace_prueba -= 15
                                                if len(elemento[2]) > 12:
                                                    texto1 = elemento[2][:12]
                                                    texto2 = elemento[2][12:]
                                                    pdf.drawString(50, yspace_prueba, texto1)
                                                    pdf.drawString(50, yspace_prueba - 15, texto2)
                                                else:
                                                    pdf.drawString(50, yspace_prueba, elemento[2])
                                                pdf.drawString(285, yspace_prueba, str(dar_formato_marca(marca3[0], tipo_medicion(elemento[1]))))
                                                pdf.drawString(470, yspace_prueba, str(marca3[3]))

                            
                        # linea separadora

                        pdf.setDash([1, 2], 0)
                        pdf.line(10, yspace_prueba-30, 585, yspace_prueba-30)

                        yspace_atleta = yspace_prueba - 30   
...
def enviar_correo_atleta(identificacion):

    for atleta in atletas:
        if atleta[0] == identificacion:
            msg = MIMEMultipart()
            message = "Hola " + ' '.join(atleta[1:4]) + ",\n\nAdjunto encontraras un archivo PDF con tus marcas en los eventos de atletismo.\n\nSaludos,\n\nAplicación Eventos de Atletismo"
            msg.attach(MIMEText(message, 'plain'))

            marcas_por_altleta_correo(identificacion)

            # setup the parameters of the message 
            password = "gruyqgdcppusqwib"
            msg['From'] = "testledvin25@gmail.com"
            msg['To'] = atleta[7]
            msg['Subject'] = "MARCAS ATLETISMO " + ' '.join(atleta[1:4])

            # add in the message body 
            with open(ruta_escritorio + '/marcas_por_atleta.pdf', 'rb') as f:
                pdf_data = f.read()
            pdf_part = MIMEApplication(pdf_data, 'pdf', Name=ruta_escritorio + 'marcas_por_atleta.pdf')
            pdf_part['Content-Disposition'] = 'attachment; filename="marcas_por_atleta.pdf"'
            msg.attach(pdf_part)
            ...
            os.remove(ruta_escritorio + '/marcas_por_atleta.pdf')
            print('Envío exitoso')

enviar_correo_atleta('3123456789')
...
def marcas_por_evento_datos():
    ...
    print('Seleccion una opcion:\n 1. Evento especifico\n 2. Marcas de todos los eventos dentro de un rango\n 0. Fin\n')
    ...
    if opcion == 1:
            ...
            # Crear un lienzo
            pdf = canvas.Canvas(ruta_escritorio + '/marcas_por_evento.pdf')
            ...
            pdf.drawString(80, 740, 'Evento:' + ' #' + str(datos_finales[0]))
            pdf.drawString(220, 740, datos_finales[1])
            ...
            for elemento4 in datos_finales[2:]:
                pdf.drawString(80, yspace_prueba, 'Prueba:' + ' ' + str(elemento4[0]))
                pdf.drawString(220, yspace_prueba, elemento4[1])
                pdf.drawString(360, yspace_prueba, 'Categoria: ' + ' ' + elemento4[2])
                pdf.drawString(500, yspace_prueba, 'Sexo:' + ' ' + elemento4[3])

                # agregar al pdf los atletas: nombre, dorsal, marca y posicion

                yspace_atleta = yspace_prueba - 15

                pdf.drawString(80, yspace_atleta, 'Nombre del atleta')
                pdf.drawString(220, yspace_atleta, 'Dorsal')
                pdf.drawString(360, yspace_atleta, 'Marca')
                pdf.drawString(500, yspace_atleta, 'Lugar')

                for id_evento in marcas:
                    if identificacion == id_evento[0]:
                        for id_prueba in id_evento[1:]:
                            if elemento4[0] == id_prueba[0]:
                                for elemento5 in id_prueba[1:]:
                                    yspace_atleta -= 15
                                    pdf.drawString(80, yspace_atleta, elemento5[1])
                                    pdf.drawString(220, yspace_atleta, str(elemento5[2]))
                                    pdf.drawString(360, yspace_atleta, str(dar_formato_marca(elemento5[0], tipo_medicion(elemento4[0]))))
    ...
    # Marcas de todos los eventos dentro de un rango de fechas
        ...
        pdf.drawString(80, 800, "APLICACIÓN EVENTOS DE ATLETISMO")
        pdf.drawString(80, 785, "MARCAS POR EVENTO")
        pdf.drawString(80, 770, "MARCAS DE TODOS LOS EVENTOS")
        ...
        for elemento in marcas_por_evento:
            for fecha_evento in eventos:
                if elemento[0] == fecha_evento[0] and validar_rango_fechas(fecha_evento[4], fecha_inicial, fecha_final ):
                    pdf.drawString(80, yspace_evento, 'Evento:' + ' #' + str(elemento[0]))
                    pdf.drawString(220, yspace_evento, fecha_evento[1])
                    ...
                    for elemento2 in elemento[1:]:
                        for elemento3 in pruebas:
                            if elemento2[0] == elemento3[0]:
                                pdf.drawString(80, yspace_prueba, 'Prueba:' + ' ' + str(elemento2[0]))
                                pdf.drawString(220, yspace_prueba, elemento3[1])
                                pdf.drawString(360, yspace_prueba, 'Categoria: ' + ' ' + elemento3[2])
                                pdf.drawString(500, yspace_prueba, 'Sexo:' + ' ' + elemento3[3])
                                break
                    ...
                    pdf.setDash([1, 2], 0)
                    pdf.line(10, yspace_evento+30, 585, yspace_evento+30)
...
def marcas_por_atleta():

    print('EVENTOS DE ATLETISMO \n \nANALISIS DE DATOS \n \nMARCAS POR ATLETAS \n')

    print('Seleccion una opcion:\n 1. Atleta especifico\n 2. Marcas de todos los eventos\n 3. Enviar resultados por correo \n 0. Salir\n ')
    ...
    if opcion == 1:
        ...
        identificacion = input('Ingrese la identificacion del atleta: ')
...
def mejores_marcas_por_prueba():
        
    marcas = ordernar_marcas()
    
    # Crear un lienzo
    pdf = canvas.Canvas(ruta_escritorio + '/mejores_marcas.pdf')
    pdf.drawString(50, 800, "APLICACIÓN EVENTOS DE ATLETISMO")
    pdf.drawString(50, 785, "MEJORES MARCAS POR PRUEBA")

    # agregar al pdf identificacion y nombre del evento

    # solicitar fecha inicial y fecha final

    fecha_inicial = '05/01/2023'#(input('Ingrese la fecha inicial "dd/mm/aaaa": '))
    fecha_final = '28/01/2023'#(input('Ingrese la fecha final "dd/mm/aaaa": '))
        ...
        for fecha_evento in eventos:
            if elemento[0] == fecha_evento[0] and validar_rango_fechas(fecha_evento[4], fecha_inicial, fecha_final ):
                ...
                for elemento2 in elemento[1:]:
                    for elemento3 in pruebas:
                        if elemento2[0] == elemento3[0]:
                            pdf.drawString(50, yspace_prueba, 'Prueba:' + ' ' + str(elemento2[0]))
                            pdf.drawString(190, yspace_prueba, elemento3[1])
                            pdf.drawString(330, yspace_prueba, 'Categoria: ' + ' ' + elemento3[2])
                            pdf.drawString(470, yspace_prueba, 'Sexo:' + ' ' + elemento3[3])
                            break

                    # agregar al pdf los atletas: nombre, dorsal, marca y posicion

                    yspace_atleta = yspace_prueba - 20

                    pdf.drawString(50, yspace_atleta, 'Nombre del atleta')
                    pdf.drawString(190, yspace_atleta, 'País')
                    pdf.drawString(300, yspace_atleta, 'Marca')
                    pdf.drawString(400, yspace_atleta, 'Posicion')
                    pdf.drawString(500, yspace_atleta, 'Evento')
                                ...
                                if elemento2[0] == id_prueba[0]:
                                    for elemento5 in id_prueba[1:]:
                                        for pais in atletas:
                                            ...
                                            if elemento5[1] == name:
                                                        yspace_atleta -= 15
                                                        pdf.drawString(50, yspace_atleta, elemento5[1])
                                                        pdf.drawString(190, yspace_atleta, pais[5])
                                                        pdf.drawString(300, yspace_atleta, str(dar_formato_marca(elemento5[0], tipo_medicion(elemento2[0]))))
                                                        pdf.drawString(400, yspace_atleta, str(elemento5[3]))
                                                        pdf.drawString(500, yspace_atleta, str(fecha_evento[0]))
