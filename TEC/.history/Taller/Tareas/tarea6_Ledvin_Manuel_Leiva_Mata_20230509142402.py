...
def pedidos_totales(lista):
    ...
    for codigo_tableta in lista:
        for cantidad_tableta in lista[codigo_tableta]:

pedidos_totales([ {'tableta 10': 1000, 'tableta 7': 850, 'tableta 10.1': 3000},
                    {'tableta 7': 600, 'tableta 10': 1500, 'tableta 8': 500} ])

Valor: unidades pedidas (entero > 0)
La función debe retornar un solo diccionario cuyos elementos tienen la misma estructura de
los diccionarios de entrada, donde cada código de tableta tendrá como valor asociado el total
de unidades pedidas a nivel de la corporación tal como se aprecia en los ejemplos del
funcionamiento:
