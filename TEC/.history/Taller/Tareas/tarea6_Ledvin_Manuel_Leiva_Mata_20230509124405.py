...
def seleccionar_proveedores(matriz1, matriz2):

    proyecto_final = {}

    for i, proveedor in enumerate(matriz1):
        for j, proyecto in enumerate(proveedor):
            if proyecto not in proyecto_final:
                proyecto_final[proyecto] = matriz2[i][j]
            else:
                proyecto_final[proyecto] += matriz2[i][j]
