...
def play():
    ...
    def changeColor(btn_id):
        numero = numeros[btn_id]

        if numero["bg"] == "SystemButtonFace":
        numero.config(bg="red", fg="white")
    ...
    for i in range(1,9):
        numero = tk.Button(numbersWrap, text=i, width=6, height=3, command=lambda num_id=i: changeColor(num_id))
