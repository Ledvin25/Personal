...
def marcas_por_evento_datos():
    ...
    # Marcas de todos los eventos dentro de un rango de fechas
        ...
        fecha_inicial = '08/10/2004'#int(input('Ingrese la fecha inicial "dd/mm/aaaa": '))
        fecha_final = '12/10/2004'#int(input('Ingrese la fecha final "dd/mm/aaaa": '))

        yspace_evento = 740

        for elemento in marcas_por_evento:
            for fecha_evento in eventos:
                if elemento[0] == fecha_evento[0] and validar_rango_fechas(fecha_evento[5], fecha_inicial, fecha_final ):
                    pdf.drawString(80, yspace_evento, 'Evento:' + ' #' + str(elemento[0]))
                    for elemento1 in eventos:
                        if elemento[0] == elemento1[0]:
                            pdf.drawString(220, yspace_evento, elemento1[1])
                            break
