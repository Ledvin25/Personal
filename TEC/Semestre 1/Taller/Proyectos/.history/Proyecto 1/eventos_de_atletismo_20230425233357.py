...
atletas = [ ['3123456789', 'Pedro', 'Pérez', 'Peraza', 'M', 'CRI', '15/10/2000', 'pedropp@gmail.com', '55556789' ], ['3023456789', 'Juan', 'Pérez', 'Peraza', 'M', 'CRI', '15/10/2000']]
...
marcas_por_evento = [[25, ['V02', ('3123456789', 2, 1302),('3023456789', 25, 1201)], ['S01', ('3123456789', 2, 1510), ('3023456789', 25, 1215)]], [28, ['S01', ], ['V02']], [29, ['V02'], ['S01']]]
...
def marcas_por_evento_datos():
    ...
    if opcion == 1:
            ...
            # convertir tuplas de marcas_por_evento a listas

            marcas_por_evento_listas = []
            marcas_final = []

            for elemento in marcas_por_evento:
                marcas_por_evento_listas.append(elemento[0])
                for elemento1 in elemento[1:]:
                    marcas_temporales = []
                    marcas_temporales.append(elemento1[0])
                    for elemento2 in elemento1[1:]:
                        marcas_temporales.append(list(elemento2))
                    marcas_por_evento_listas.append(marcas_temporales)
                    marcas_final.append(marcas_por_evento_listas)

            marcas_por_evento_listas = [marcas_por_evento_listas]
            ...
            while verify:
                    ...
                    for elemento in marcas_por_evento_listas:
                        if identificacion == elemento[0]:
                            verify = False
            ...
            # ordenar marcas de menor a mayor si el tipo de medicion es 'T' y de mayor a menor si es 'M' y agregar a datos_finales con el codigo de la prueba

            for elemento4 in elemento[1:]:
                for elemento5 in pruebas:
                    for elemento6 in disciplinas:
                        if elemento5[4] == elemento6[0]:
                            for codigo_prueba in datos_finales[2:]:
                                if elemento4[0] == codigo_prueba[0]:
                                    if elemento6[1] == 'T':
                                        datos_finales[datos_finales.index(codigo_prueba)].append(elemento4[2])
                                        datos_finales[datos_finales.index(codigo_prueba)][-1].sort()
                                    elif elemento6[1] == 'M':
                                        datos_finales[datos_finales.index(codigo_prueba)].append(elemento4[2])
                                        datos_finales[datos_finales.index(codigo_prueba)][-1].sort(reverse=True)
                                    break
