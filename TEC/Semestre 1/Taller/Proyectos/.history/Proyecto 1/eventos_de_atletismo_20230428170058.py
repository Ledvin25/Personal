...
def marcas_por_atleta():
    ...
    if opcion == 1:
        ...
        for elemento in marcas_por_atleta:
            if elemento[0] == identificacion:
                pdf.drawString(80, yspace_atleta, 'Atleta:' + ' #' + str(elemento[0]))
                pdf.drawString(220, yspace_atleta, elemento[1])

                # agregar al pdf las marcas del atleta

                yspace_evento = yspace_atleta - 15

                pdf.drawString(80, yspace_evento, 'Evento')
                pdf.drawString(220, yspace_evento, 'Prueba')
                pdf.drawString(360, yspace_evento, 'Marca')
                pdf.drawString(500, yspace_evento, 'Lugar')

                for id_evento in marcas:
                    for id_prueba in id_evento[1:]:
                        for elemento2 in id_prueba[1:]:
                            if identificacion == elemento2[2]:
                                for elemento3 in eventos:
                                    if id_evento[0] == elemento3[0]:
                                        for elemento4 in pruebas:
                                            if id_prueba[0] == elemento4[0]:
                                                yspace_evento -= 15
                                                pdf.drawString(80, yspace_evento, elemento3[1])
                                                pdf.drawString(220, yspace_evento, elemento4[1])
                                                pdf.drawString(360, yspace_evento, str(dar_formato_marca(elemento2[0], tipo_medicion(elemento4[0]))))
                                                pdf.drawString(500, yspace_evento, str(elemento2[3]))

                yspace_atleta = yspace_evento - 30
