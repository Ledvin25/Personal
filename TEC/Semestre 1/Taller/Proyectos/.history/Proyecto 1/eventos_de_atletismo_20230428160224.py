...
def marcas_por_atleta():
    ...
    if opcion == 1:
        ...
        identificacion = 2023071280#int(input('Ingrese la identificacion del atleta: '))

        # encontrar nombre del atleta

        for nombre in atletas:
            if identificacion == nombre[0]:
                pdf.drawString(80, 740, 'Atleta:' + ' ' + nombre[1])
                break
        ...
        for elemento in marcas:
            for elemento2 in elemento[1:]:
                for elemento3 in elemento2[1:]:
                    if identificacion == elemento3[1]:
                        pdf.drawString(80, yspace_atleta, 'Atleta:' + ' ' + str(elemento[0]))

                        # agregar al pdf las marcas del atleta: evento, prueba, marca y posicion
                            ...
                            if identificacion == id_evento[0]:
                                for id_prueba in id_evento[1:]:
                                    for elemento2 in id_prueba[1:]:
                                        yspace_evento -= 15
                                        pdf.drawString(80, yspace_evento, str(id_evento[0]))
                                        pdf.drawString(220, yspace_evento, str(elemento2[0]))
                                        pdf.drawString(360, yspace_evento, str(dar_formato_marca(elemento2[1], tipo_medicion(elemento2[0]))))
                                        pdf.drawString(500, yspace_evento, str(elemento2[2]))
