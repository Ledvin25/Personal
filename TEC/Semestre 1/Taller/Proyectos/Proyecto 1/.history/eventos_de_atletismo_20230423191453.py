...
def verificar_correo_electronico(correo_electronico):
    # Validar el formato del correo electrónico
    es_valido = validate_email.validate_email(correo_electronico)
    ...
    if es_valido:
        ...
        try:
            # Consultar el registro MX para el dominio
            records = dns.resolver.query(dominio, 'MX')
            if len(records) > 0:
                # Si se encontraron registros MX, entonces el dominio existe
                return True
        except dns.resolver.NXDOMAIN:
            # Si no se encontró ningún registro para el dominio, entonces el dominio no existe
            return False
    
    # Si el correo electrónico no es válido o no se encontró registro MX, entonces el correo electrónico es inválido
    return False
