...
sumar_resta_matrices([[1,2 ],[3, 4]],[[5, 6],[7, 8]], '+')         
...
''''def multiplica_matrices(A,B):
    ...
    if len (B) != len(A[0]):
        ...
        for fila_A in A:
            matriz_multiplicacion = 0
            for columna_A in fila_A:
                for fila_B in B:
                    for columna_B in fila_B:
                        multiplicacion = columna_A * columna_B
                        matriz_multiplicacion += multiplicacion
                        break
                    break

            result.append(matriz_multiplicacion)
