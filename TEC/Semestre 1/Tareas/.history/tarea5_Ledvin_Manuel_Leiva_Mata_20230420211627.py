...
# Ejercicio 16 - coautor Joseph Arrieta
...
def triángulo_de_pascal(n):
    ...
    triangulo = [[1]]
    #agrega numero al triangulo segun requerimientos
    for i in range(n-1):
...
def multiplica_matrices(A,B):
    ...
    if len (B) != len(A[0]):
        return 'La matriz A debe tener la misma cantidad de columnas que filas de la matriz B'
    else:
        for fila_A in A:
            multi_matrix = []
            ...
            for columna_A in fila_A:
                for fila_B in B:
                    ...
                    for columna_B in fila_B:
                        multi.append(columna_A*columna_B)
                    multi_matrix.append(multi)
                result1 = sumar_resta_matrices(multi_matrix)

#funcion para sumar terminos
def sumar_terminos(listas):
    sumas = []
