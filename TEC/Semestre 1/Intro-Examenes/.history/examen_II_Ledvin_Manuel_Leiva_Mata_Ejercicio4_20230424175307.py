...
def sopa_de_letras(lista, palabra):
    ...
    pl = palabra.count(letra)
    ps = word_list.count(letra)
        ...
        for letra in palabra:
            for i, element in enumerate(lista):
                for j, element2 in enumerate(element):
                    if letra == element2 and (i, j) not in tupla and len(tupla) <= len(palabra) and letra not in word_list and palabra.count(letra) >= word_list.count(letra):
                        if 
