            ...
            for i, element in enumerate(lista):
                for j, element2 in enumerate(element):
                    if letra == element2 and (i, j) not in tupla and len(tupla) <= len(palabra) and letra not in word_list :
                        ...
                        tupla.append((i, j))
                        break
                    elif letra == element2 and (i, j) not in tupla and len(tupla) <= len(palabra) and letra in word_list and palabra.count(letra) > word_list.count(letra) and word_list[len(word_list) - 1] == palabra[(len(word_list) - 1)]:
                        p = word_list[len(word_list) - 1]
                        s = palabra[(len(word_list) - 1)]
                        word_list.append(letra)
                        tupla.append((i, j))
                        break
                    else:
