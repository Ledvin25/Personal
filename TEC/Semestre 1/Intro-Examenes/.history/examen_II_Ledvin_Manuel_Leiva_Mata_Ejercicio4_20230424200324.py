...
def sopa_de_letras(lista, palabra):
    ...
    for i in range(filas): # recorrer filas
        for j in range(columnas): # recorrer columnas
            if lista[i][j] == palabra[0]: # si la letra es igual a la primera letra de la palabra
                for diagonal_i in range(-1, 2): # recorrer diagonales
                    for diagonal_j in range(-1, 2): # recorrer diagonales
                        if diagonal_i != 0 or diagonal_j != 0: 
                            ...
                            for k in range(1, len(palabra)): # recorrer la palabra
                                fila = i + k * diagonal_i # fila de la palabra
                                columna = j + k * diagonal_j # columna de la palabra
                                if filas <= fila < 0 or columnas <= columna < 0 or lista[fila][columna] != palabra[k]: # si la palabra no esta en la sopa de letras
...
print(sopa_de_letras([ 'DAMEMORIAO', 'RZPUZLEÁÚY', 'DCPFBJLYDR', 'MSOPAOBSLJ', 'EBZSÑOGEUJ', 'NMQAHRTKDC', 'TMPNARYWST', 'ESYAAIUPWJ', 'EPVSJUODTW', 'ZARBALAPÓO'],'MEMORIA' ))
