...
def play():
    ...
    def comprobarSumas():
        ...
        # Comprobar columnas
        for columna in casillas.values():
    ...
    for lenCasillas in nivel_Casillas:
        if lenCasillas[0] == 1:
            for i in range(lenCasillas[4]): # Crear casillas horizontales
                i += lenCasillas[3] + 1
                casilla = tk.Button(Tablero, width=6, height=3, command=lambda num_id=(lenCasillas[2],i): colocarNumero(num_id), bg="white", font=("Eras Demi ITC", 12))
