...
def settings():
    ...
    def errorWindow(text1, tiempo):      
        
        errorWindow = tk.Toplevel()
        errorWindow.title("Error")
        errorWindow.geometry(f"600x100+{errorWindow.winfo_screenwidth()//2-150}+{errorWindow.winfo_screenheight()//2-50}")
        errorWindow.resizable(0,0)

        errorLabel = tk.Label(errorWindow, text=text1 + 'NO PUEDE SER MAYOR QUE' + tiempo, font=("Eras Demi ITC", 12))
    ...
    def tiempoLimite():
        ...
        if time[0] > 2 and clockVar == 2:
            errorWindow("LAS HORAS ", "2")
        elif 0 <= time[1] <= 23 and clockVar == 2:
            errorWindow("LOS MINUTOS ", "ENTRE 0 Y 23")
        elif 0 <= time[2] <= 59 and clockVar == 2:
            errorWindow("LOS SEGUNDOS ", "ENTRE 0 Y 59")
        else:
