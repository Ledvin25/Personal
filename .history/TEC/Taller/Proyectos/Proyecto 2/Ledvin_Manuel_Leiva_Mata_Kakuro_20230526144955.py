...
def play():
    ...
    def iniciarJuego():
        ...
        if playerName.get() == "":
            ...
            error = tk.Label(Error1, text="DEBE INGRESAR UN NOMBRE", font=("Eras Demi ITC", 12))
            error.pack(pady=10)
    ...
    def top10(state):
        ...
        top = tk.Tk()
        top.title("TOP 10")
        top.geometry(f"800x900+{top.winfo_screenwidth()//2-400}+{top.winfo_screenheight()//2-450}")
        ...
        for i, level in enumerate(top10file):

            # Crear frame para cada nivel y usarlo como cuadricula
            frame = tk.Frame(levels)
            ...
            nivel = tk.Label(frame, text=level, font=("Eras Demi ITC", 12))
            nivel.grid(row=0, column=0, padx=10, pady=10)

            # Crear cuadricula para top 10 - encabezado

            pos = tk.Label(frame, text="Posicion", font=("Eras Demi ITC", 12))
            pos.grid(row=1, column=0, padx=10, pady=10)

            name = tk.Label(frame, text="Nombre", font=("Eras Demi ITC", 12))
            name.grid(row=1, column=1, padx=10, pady=10)

            time = tk.Label(frame, text="Tiempo", font=("Eras Demi ITC", 12))
            time.grid(row=1, column=2, padx=10, pady=10)


            for i, position in enumerate(top10file[level]):
                pos = tk.Label(frame, text=position[0], font=("Eras Demi ITC", 12))
                pos.grid(row=i+2, column=0, padx=10, pady=2)

                name = tk.Label(frame, text=position[1], font=("Eras Demi ITC", 12))
                name.grid(row=i+2, column=1, padx=10, pady=2)

                time = tk.Label(frame, text= str(position[2]//3600) + ":" + str(position[2]//60) + ":" + str(position[2]%60), font=("Eras Demi ITC", 12)) 
                time.grid(row=i+2, column=2, padx=10, pady=2)

        # Boton de cerrar
        cerrar = tk.Button(top, text="Cerrar", font=("Eras Demi ITC", 12), command=top.destroy)
        cerrar.pack(pady=10)
    ...
    top10btn = tk.Button(playWindow, text="TOP 10", width=15, height=2, font=("Eras Demi ITC", 12), background='#FFFF00', command=lambda: top10(1)
