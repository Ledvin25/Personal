...
def play():
    ...
    def top10():
        ...
        with open('TEC/Taller/Proyectos/Proyecto 2/kakuro2023top10.dat', 'rb') as file:
            ...
            for i, position in enumerate(top10[category]):
                if elapsed_time < position[2]:
                    top10[category].insert(i, [position[0],'Ledvinasdas', elapsed_time])
                    top10[category].pop()
        ...
        encabezado = tk.Frame(tabla)
        encabezado.pack()

        # Crear etiquetas de encabezado

        nivel = tk.Label(encabezado, text="Nivel", font=("Eras Demi ITC", 12))
        nivel.pack(side=tk.LEFT, padx=10)

        nombre = tk.Label(encabezado, text="Nombre", font=("Eras Demi ITC", 12))
