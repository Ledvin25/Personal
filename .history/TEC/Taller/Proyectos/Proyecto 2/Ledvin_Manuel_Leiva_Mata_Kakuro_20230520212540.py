...
def play():
    ...
    def ventanaConfirmar(text):

        global state

        confirmWindow = tk.Tk()
        confirmWindow.title(text + 'JUEGO')
        ...
        confirmacion = tk.Label(confirmWindow, text=f"¿ESTA SEGURO DE {text} EL JUEGO?", font=("Eras Demi ITC", 12))
        confirmacion.pack(pady=10)

        # Boton de aceptar
        aceptar = tk.Button(confirmWindow, text="SI", font=("Eras Demi ITC", 12), command= lambda: state = True)
        aceptar.pack(pady=10)

        # Boton de cancelar
        cancelar = tk.Button(confirmWindow, text="NO", font=("Eras Demi ITC", 12), command= state= False)
    ...
    for i in range(9):
        for j in range(9):
            casilla = tk.Button(Tablero, width=6, height=3, command=lambda num_id=(i,j): colocarNumero(num_id), bg="white", font=("Eras Demi ITC", 12))
    ...
    borrarJuego = tk.Button(playWindow, text="BORRAR JUEGO", width=15, height=2, font=("Eras Demi ITC", 12), background='#8DB3E2', command=deleteJuego)
    borrarJuego.grid(row=2, column=2, padx=30)
    terminarJuego = tk.Button(playWindow, text="TERMINAR JUEGO", width=15, height=2, font=("Eras Demi ITC", 12), background='#00B050', command=endGame)
