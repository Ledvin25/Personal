...
def play():
    ...
    def borrarNumero():
        ...
        if running:
            if casillaSeleccionada != None:
                ...
                accionesPasadas.append([casillaSeleccionada, casillaSeleccionada["text"]])
                print(accionesPasadas)
                ...
                listaAcciones.append('borrarNumero')
    ...
    def deshacerAccion():
        ...
        if running:
            ...
            ultimaAccion = listaAcciones.pop()

            if len(accionesPasadas) > 0 and ultimaAccion == 'colocarNumero':
                accion = accionesPasadas.pop()
                accionesFuturas.append(accion)
                accion[0].config(text='')
            elif len(accionesPasadas) > 0 and ultimaAccion == 'borrarNumero':
                accion = accionesPasadas.pop()
                accionesFuturas.append(accion)
    ...
    def rehacerAccion():
        ...
        if running:
            ...
            if len(accionesFuturas) > 0 and ultimaAccion == 'colocarNumero':
                accion = accionesFuturas.pop()
                accionesPasadas.append(accion)
                accion[0].config(text=accion[1])
            elif len(accionesFuturas) > 0 and ultimaAccion == 'borrarNumero':
                accion = accionesFuturas.pop()
                accionesPasadas.append(accion)
                accion[0].config(text='')
    ...
    def colocarNumero(btn_id):
        ...
        if running:
            ...
            if numero != None:
                ...
                accionesPasadas.append([casilla, numero['text']])

                for numero in numeros.values():
                    if numero["bg"] == "#00CCCC":
                        numero.config(bg="white", fg="black")
                
                listaAcciones.append('colocarNumero')
