...
def play():
    ...
    def colocarNumero(btn_id):
        ...
        if running:
            if numero != None:
                ...
                accionesPasadas.append([btn_id, numero['text']])
    ...
    def deshacerAccion():
        ...
        if running:
            if len(accionesPasadas) > 0:
                accion = accionesPasadas.pop()
                accionesFuturas.append(accion)
                casillaSeleccionada = casillas[accion[0]]
                casillaSeleccionada.config(text='')
    ...
    def rehacerAccion():
        ...
        if running:
            if len(accionesFuturas) > 0:
                ...
                casillaSeleccionada = casillas[accion[0]]
                casillaSeleccionada.config(text=accion[1])
    ...
    for i in range(1,10):
        numero = tk.Button(numbersWrap, text=i, width=4, height=3, command=lambda num_id=i: seleccionarNumero(num_id), background='white', font=("Eras Demi ITC", 12))
