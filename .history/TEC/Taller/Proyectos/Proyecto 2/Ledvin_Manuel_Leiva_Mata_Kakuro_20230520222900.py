...
def play():
    ...
    # Establecer tablero de juego y sus casillas ------------------------------------------------------------------------------------------------------------------------------:
    casillasBloqueadas = [(1, 1), (1, 2), (1, 3), (1, 4), (1, 5)]
    ...
    def crear_texto(canvas, x, y, texto):
        canvas.create_text(x, y, text=texto, font=("Arial", 12), fill="black")  # Dibujar el texto

    for i in range(9):
        for j in range(9):
            if (i,j) in casillasBloqueadas:
                casilla = tk.Button(Tablero, width=6, height=3)
                casilla.grid(row=i, column=j)

        # Dibujar la división diagonal
    crear_diagonal(canvas, 0, 0, 200, 200)

    # Colocar el número sobre la diagonal
    crear_texto(canvas, 100, 100, "8")

    # Colocar el número bajo la diagonal
    crear_texto(canvas, 100, 150, "2")

      # Coordenadas de las casillas bloqueadas
    ...
    for i in range(9):
        for j in range(9):
            if (i,j) not in casillasBloqueadas:
