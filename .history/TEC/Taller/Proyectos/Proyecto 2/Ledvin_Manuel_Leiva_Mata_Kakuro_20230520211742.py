...
def play():
    ...
    def ventanaConfirmar(text):
        ...
        playWindow.geometry(f"300x100+{playWindow.winfo_screenwidth()//2-150}+{playWindow.winfo_screenheight()//2-50}")
        playWindow.resizable(False, False)

        # Crear etiqueta de confirmacion
        confirmacion = tk.Label(playWindow, text=f"¿ESTA SEGURO DE {text} EL JUEGO?", font=("Eras Demi ITC", 12))
        confirmacion.pack(pady=10)

        # Boton de aceptar
        aceptar = tk.Button(playWindow, text="Aceptar", font=("Eras Demi ITC", 12), command=playWindow.destroy)
        aceptar.pack(pady=10)
