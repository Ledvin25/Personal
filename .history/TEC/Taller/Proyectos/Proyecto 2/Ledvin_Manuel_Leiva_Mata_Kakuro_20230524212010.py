...
iDificulty = 1
partidas = {"FÁCIL": (
        ((2,9,1,9,2),(2,4,1,8,2),(2,14,2,7,4),
        (1,3,2,7,2),(2,38,2,6,6),(2,22,2,5,6),(1,25,3,4,5),
        (1,6,4,4,3),(2,29,4,4,4),(2,7,3,3,3),(2,4,3,2,2),
        (1,3,4,1,2),(1,29,5,1,7),(1,42,6,2,7),(1,24,7,3,3),
        (2,8,7,3,2),(2,10,7,2,2),(1,35,8,1,5),(1,3,9,1,2),
        (1,12,7,7,2),(2,24,4,8,3),(2,13,5,9,2),
        (1,0,1,1,0),(1,0,1,2,0),(1,0,1,3,0),(1,0,1,4,0),
        (1,0,1,5,0),(1,0,1,6,0),(1,0,1,7,0),(1,0,2,1,0),
        (1,0,2,2,0),(1,0,2,3,0),(1,0,2,4,0),(1,0,3,1,0),
        (1,0,6,1,0),(1,0,7,1,0),(1,0,9,4,0),(1,0,9,5,0),
        (1,0,9,6,0),(1,0,9,7,0),(1,0,8,8,0),(1,0,8,9,0),
        (1,0,9,8,0),(1,0,9,9,0),(1,0,8,7,0),(1,0,9,8,0),
        (1,0,9,9,0),(1,0,4,9,0)),
                    ),
            } 
...
def play():
    ...
    def escogerNivel():
        ...
        if iDificulty == 1:
                nivelRandom = random.randrange(0, len(partidas['FÁCIL']))
                nivel_Casillas = partidas['FÁCIL'][nivelRandom]
        elif iDificulty == 2:
            nivelRandom = random.randrange(0, len(partidas['MEDIO']))
            nivel_Casillas = partidas['MEDIO'][nivelRandom]
        elif iDificulty == 3:
            nivelRandom = random.randrange(0, len(partidas['DIFÍCIL']))
            nivel_Casillas = partidas['DIFÍCIL'][nivelRandom]
        elif iDificulty == 4:
            nivelRandom = random.randrange(0, len(partidas['EXPERTO']))
            nivel_Casillas = partidas['EXPERTO'][nivelRandom]
    ...
    def saveGame():
        ...
        # datos a guardar
        saved = [casillas_save, accionesPasadas_save, accionesFuturas_save, segundos['text'], minutos['text'], horas['text'], nivel_Casillas]
        ...
        save.geometry(f"300x100+{save.winfo_screenwidth()//2-150}+{save.winfo_screenheight()//2-50}")
        ...
        si = tk.Button(save, text="SI", font=("Eras Demi ITC", 12), command=save.destroy)
        si.pack(side=tk.LEFT, padx=10, pady=10)

        no = tk.Button(save, text="NO", font=("Eras Demi ITC", 12), command=save.destroy)
    ...
    def loadGame():
        ...
        accionesPasadas_save = saved[1]
        accionesFuturas_save = saved[2]
        segundos_save = saved[3]
        minutos_save = saved[4]
        horas_save = saved[5]
        nivel_Casillas = saved[6]
        ...
        # cargar acciones pasadas

        for accion in accionesPasadas_save:
            casilla = casillas[(accion[0][0], accion[0][1])]
            accionesPasadas.append([casilla, accion[1], accion[2]])

        # cargar acciones futuras

        for accion in accionesFuturas_save:
            casilla = casillas[(accion[0][0], accion[0][1])]
            accionesFuturas.append([casilla, accion[1], accion[2]])
...
def settings():
    ...
    cerrar = tk.Button(settingsWindow, text="CERRAR", width=15, height=2, font=("Eras Demi ITC", 12), background='#FF0066', command=settingsWindow.destroy)

    # entrada cronometro, horas, minutos, segundos
    horas = tk.Entry(settingsWindow, width=5, font=("Eras Demi ITC", 12))
    minutos = tk.Entry(settingsWindow, width=5, font=("Eras Demi ITC", 12))
    segundos = tk.Entry(settingsWindow, width=5, font=("Eras Demi ITC", 12))

    # posicionamiento
    dificultyLabel.place(x=250, y=100, anchor="center")
    dificultyBtn.place(x=250, y=180, anchor="center")
    cronometroLabel.place(x=250, y=300, anchor="center")
    horasLabel.place(x=150, y=350, anchor="center")
    horas.place(x=150, y=400, anchor="center")
    minutosLabel.place(x=250, y=350, anchor="center")
    minutos.place(x=250, y=400, anchor="center")
    segundosLabel.place(x=350, y=350, anchor="center")
    segundos.place(x=350, y=400, anchor="center")
    cerrar.place(x=250, y=500, anchor="center")
