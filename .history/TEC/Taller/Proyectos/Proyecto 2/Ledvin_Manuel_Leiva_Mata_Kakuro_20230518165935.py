...
def play():
    ...
    def seleccionarNumero(btn_id):
        ...
        numero = numeroSeleccionado = numeros[btn_id]
    ...
    def colocarNumero(btn_id):
        ...
        if numeroSeleccionado != None:
            numero = numeroSeleccionado["text"]
            casilla = casillas[btn_id]
            casilla.config(text=numero)
    ...
    for i in range(9):
        for j in range(9):
            casilla = tk.Button(Tablero, width=8, height=4, command=lambda num_id=i: colocarNumero(num_id))
