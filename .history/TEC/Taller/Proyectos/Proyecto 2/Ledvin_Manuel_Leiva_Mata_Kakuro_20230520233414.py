    ...
    for lenCasillas in nivel_Casillas:
        if lenCasillas[0] == 1:
            for i in range(lenCasillas[4]):
                casilla = tk.Button(Tablero, width=6, height=3, command=lambda num_id=(lenCasillas[2],j): colocarNumero(num_id), bg="white", font=("Eras Demi ITC", 12))
                casilla.grid(row=lenCasillas[2], column=j)
                casillas[(lenCasillas[2],j)] = casilla
        elif lenCasillas[0] == 2:
            for j in range(lenCasillas[4]):
                casilla = tk.Button(Tablero, width=6, height=3, command=lambda num_id=(i,lenCasillas[3]): colocarNumero(num_id), bg="white", font=("Eras Demi ITC", 12))
