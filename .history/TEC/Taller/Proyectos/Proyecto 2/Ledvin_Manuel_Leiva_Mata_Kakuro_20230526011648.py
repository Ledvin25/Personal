...
def play():
    ...
    def top10():
        ...
        with open('TEC/Taller/Proyectos/Proyecto 2/kakuro2023top10.dat', 'rb') as file:
            ...
            for i, position in enumerate(top10[category]):
                if elapsed_time < position[2]:
                    top10[category].insert(i, [position[0],playerName.get(), elapsed_time])
                    top10[category].pop()
        ...
        top = tk.Tk()
        top.title("TOP 10")
        top.geometry(f"700x300+{top.winfo_screenwidth()//2-350}+{top.winfo_screenheight()//2-150}")
        ...
        for level in top10:
            ...
            for position in top10[level]:
                ...
                # Tiempo
                textTime = position[2]//3600 + ":" + position[2]//60 + ":" + position[2]%60
    ...
    top10btn = tk.Button(playWindow, text="TOP 10", width=15, height=2, font=("Eras Demi ITC", 12), background='#FFFF00')
